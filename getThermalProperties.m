...
function [M] = getThermalProperties(M,MAT)

% Get thermal properties assuming solid ice for nodes
% Constants
T0    = 270;      % Ref temp for laws [K] -- hardcoded with constants below
a0    = 1.704e-4; % Ref thermal expansivity [1/K]

% Density
M.alpha = a0 * M.T / T0;                           % Thermal expansivity [1/K]
M.rho   = (1-M.phi).*MAT.rho0.*(1-(M.T-T0).*M.alpha);  % Density [kg/m^3]
...
% if M.vRes>0
%     rhoMelt((M.r>= M.rResBot) & (M.r<=M.rResTop)) = M.rhoRes; 
% end
% 
% CpMelt = zeros(size(M.Cp));
% CpMelt(M.r<=M.rOcnTop) = M.CpOcn;
% if M.vRes>0
%     CpMelt(M.r>= M.rResBot & M.r<=M.rResTop) = M.CpRes; 
% end
% 
% k_melt = 50*ones(size(M.k)); % Melt thermal conductivity
% 
...
% 
% % Get Conductivity via volume
% M.k = (vfm./k_melt + (1-vfm)./M.k).^-1;
% 
...
% M.Cp  = M.Cp .* (1-mfm) + CpMelt .* mfm; % Specific heat capacity
...
% Viscosity
%%%%%%%%%%%%%
M.eta = MAT.eta0 * exp((MAT.Ea/(MAT.R*M.Tm_ocn))*(M.Tm_ocn./M.T-1));
